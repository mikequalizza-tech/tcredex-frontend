...
- **Middleware fast-path**: `middleware.ts` now bypasses Clerk/auth processing for `/api` requests so backend-owned routes are not blocked or slowed by edge middleware.
...
- Middleware still handles QR/referral redirects and onboarding gating for app pages; it no longer intercepts API calls.
