-- =============================================================================
-- Migration 033: Fix RPC function column name references
-- =============================================================================
-- The RPC functions reference s.poverty_rate, s.mfi_percent, s.unemployment_rate
-- but master_tax_credit_sot has nmtc_poverty_rate, nmtc_mfi_percent, nmtc_unemployment_rate
-- =============================================================================
...
-- =============================================================================
-- Recreate tile function with correct column names
-- =============================================================================
...
-- =============================================================================
-- Update tract_map_layer view with correct column names
-- =============================================================================
...
SELECT
    ...
    s.nmtc_poverty_rate as poverty_rate,
    s.nmtc_mfi_percent as mfi_pct,
    s.nmtc_unemployment_rate as unemployment_rate
FROM tract_geometries g
LEFT JOIN master_tax_credit_sot s ON g.geoid = s.geoid;
...
-- =============================================================================
-- Fix get_tract_with_credits - use correct column names
-- =============================================================================
...
BEGIN
    ...
    SELECT
        ...
        COALESCE(s.has_state_lihtc, false) as has_state_lihtc,
        COALESCE(s.stack_score, 0) as stack_score,
        s.nmtc_poverty_rate::DOUBLE PRECISION as poverty_rate,
        s.nmtc_mfi_percent::DOUBLE PRECISION as mfi_pct,
        s.nmtc_unemployment_rate::DOUBLE PRECISION as unemployment_rate
    FROM tract_geometries g
    LEFT JOIN master_tax_credit_sot s ON g.geoid = s.geoid
...
-- =============================================================================
-- Fix get_tract_at_point - use correct column names
-- =============================================================================
...
-- =============================================================================
-- Fix get_map_tracts_in_bbox - use correct column names
-- =============================================================================
...
BEGIN
    ...
    SELECT
        ...
        s.nmtc_poverty_rate::DOUBLE PRECISION as poverty_rate,
        s.nmtc_mfi_percent::DOUBLE PRECISION as median_family_income_pct
    FROM tract_geometries g
    LEFT JOIN master_tax_credit_sot s ON g.geoid = s.geoid
