-- =============================================================================
-- FIX ELIGIBILITY - Remove HTC and Brownfield from has_any_tax_credit calculation
-- =============================================================================
-- HTC and Brownfield logic is NOT implemented yet.
-- This migration updates the vector tile function to exclude them from eligibility.
-- =============================================================================
...
-- =============================================================================
-- Recreate tile function WITHOUT HTC or Brownfield in eligibility calculation
-- =============================================================================
...
BEGIN
    ...
    FROM (
        SELECT
            g.geoid,
            -- Eligibility WITHOUT HTC or Brownfield (not implemented yet)
            CASE WHEN (
                COALESCE(s.is_nmtc_eligible, false) OR
                COALESCE(s.is_lihtc_qct_2025, false) OR
                COALESCE(s.is_lihtc_qct_2026, false) OR
                COALESCE(s.is_oz_designated, false) OR
                COALESCE(s.is_dda_2025, false) OR
                COALESCE(s.is_dda_2026, false) OR
                COALESCE(s.has_state_nmtc, false)
...
COMMENT ON FUNCTION get_vector_tile IS 'Generate MVT tile - eligibility excludes HTC and Brownfield (not implemented)';

-- =============================================================================
-- Update tract_map_layer view to recalculate has_any_tax_credit
-- =============================================================================
...
SELECT
    ...
    -- Recalculate eligibility WITHOUT HTC or Brownfield
    CASE WHEN (
        COALESCE(s.is_nmtc_eligible, false) OR
        COALESCE(s.is_lihtc_qct_2025, false) OR
        COALESCE(s.is_lihtc_qct_2026, false) OR
...
COMMENT ON VIEW tract_map_layer IS 'Tract map layer - eligibility excludes HTC and Brownfield (not implemented)';

-- =============================================================================
-- Update RPC functions to use recalculated eligibility
-- =============================================================================
...
BEGIN
    ...
    SELECT
        g.geoid,
        ST_AsGeoJSON(g.geometry)::TEXT as geom_json,
        -- Eligibility WITHOUT HTC or Brownfield
        CASE WHEN (
            COALESCE(s.is_nmtc_eligible, false) OR
            COALESCE(s.is_lihtc_qct_2025, false) OR
            COALESCE(s.is_lihtc_qct_2026, false) OR
...
COMMENT ON FUNCTION get_map_tracts_in_bbox IS 'Get tracts in bbox - removed HTC/Brownfield columns';
